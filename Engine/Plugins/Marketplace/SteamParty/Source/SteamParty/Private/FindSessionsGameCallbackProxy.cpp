...
#include "FindSessionsGameCallbackProxy.h"
...
//////////////////////////////////////////////////////////////////////////
// UFindSessionsGameCallbackProxy

UFindSessionsGameCallbackProxy::UFindSessionsGameCallbackProxy(const FObjectInitializer& ObjectInitializer)
...
UFindSessionsGameCallbackProxy* UFindSessionsGameCallbackProxy::FindSessionsGame(UObject* WorldContextObject, class APlayerController* PlayerController, int MaxResults, bool bUseLAN, const FString& InGameType)
{
	UFindSessionsGameCallbackProxy* Proxy = NewObject<UFindSessionsGameCallbackProxy>();
...
void UFindSessionsGameCallbackProxy::Activate()
{
	if (PlayerControllerWeakPtr.IsValid() && PlayerControllerWeakPtr.Get()->PlayerState && PlayerControllerWeakPtr.Get()->PlayerState->UniqueId.GetUniqueNetId().IsValid())
	{
		...
		if (Sessions.IsValid())
		{
			...
			if (GameType == FString(TEXT("ALL")))
			{
				...
				SearchObject->QuerySettings.Set(SETTING_GAMEMODE, FString(TEXT("PARTY")), EOnlineComparisonOp::NotEquals);
			}
			else
			{
...
void UFindSessionsGameCallbackProxy::OnCompleted(bool bSuccess)
{
	if (PlayerControllerWeakPtr.IsValid() && PlayerControllerWeakPtr.Get()->PlayerState && PlayerControllerWeakPtr.Get()->PlayerState->UniqueId.GetUniqueNetId().IsValid())
	{
		auto Sessions = Online::GetSessionInterface();
...
FString UFindSessionsGameCallbackProxy::GetGameType(const FBlueprintSessionResult& Result)
{
